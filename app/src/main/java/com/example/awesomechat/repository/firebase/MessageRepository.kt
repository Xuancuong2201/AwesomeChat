...
class MessageRepository @Inject constructor(
    ...
    override suspend fun sentMessage(recipient: String, content: String) {
        ...
        val message = DetailMessage(
            content = content,
            sentby = idUser,
            status = false,
            time = Timestamp.now().toDate(),
            type = InfoFieldQuery.TYPE_MESS
        )
        db.collection(InfoFieldQuery.COLLECTION_MESSAGES).document(getIdMess(idUser, idRecipient))
            .collection(InfoFieldQuery.KEY_MESSAGE)
            .add(message).await()
    }
    ...
    override suspend fun createConversation(idUser: String, idRecipient: String): String {
        ...
        val conversation =
            Conversation(id, InfoFieldQuery.TYPE_DOUBLE, arrayListOf(idUser, idRecipient))
    ...
    override suspend fun fetchIdUser(email: String): String? {
        return withContext(Dispatchers.IO) {
            try {
                val idUser =
                    db.collection(InfoFieldQuery.COLLECTION_USER)
                        .whereEqualTo(InfoFieldQuery.KEY_EMAIL, email).get()
    ...
    override suspend fun getTokenUser(email: String): String? {
        return withContext(Dispatchers.IO) {
            try {
                val token = db.collection(InfoFieldQuery.COLLECTION_USER)
                    .whereEqualTo(InfoFieldQuery.KEY_EMAIL, email).get()
                    .await().documents.firstNotNullOfOrNull { it.getString("token") }
                token
            } catch (exception: Exception) {
                null
            }
    ...
    override suspend fun listenerChangeDetailsMessage(recipient: String): Flow<List<DetailMessage>> =
        callbackFlow {
            ...
            if (idMess != null) {
                val docRef = db.collection(InfoFieldQuery.COLLECTION_MESSAGES).document(idMess)
                    .collection(InfoFieldQuery.KEY_MESSAGE)
    ...
    private suspend fun getIdMess(idUser: String, idRecipient: String): String {
        val idMessQuerySnapshot =
            db.collection(InfoFieldQuery.COLLECTION_CONVERSATION)
                .whereArrayContains(InfoFieldQuery.KEY_USER, idUser)
